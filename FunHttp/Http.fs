...
module HttpEncodings = 
    ...
    let internal getEncoding (encodingStr:string) = Encoding.GetEncoding encodingStr
...
module private HttpHelpers =
    ...
    let getProperty (typ:Type) obj prop =        
        let prop = try typ.GetTypeInfo().GetProperty prop with _ -> null
    ...
    let (?<-) obj prop value =
        let prop = obj.GetType().GetTypeInfo().GetProperty(prop)
        if not (isNull(prop)) && prop.CanWrite then
            try 
    ...
    let reraisePreserveStackTrace (e:Exception) =
        try
            let remoteStackTraceString = typeof<exn>.GetTypeInfo().GetField("_remoteStackTraceString", BindingFlags.Instance ||| BindingFlags.NonPublic);
...
type Http private() =
    ...
    static let regexOptions = RegexOptions.Compiled
    ...
    static member private InnerRequest
        ...
        let uri = Uri(Http.AppendQueryToUrl(url, defaultArg query []))
